...
source(paste0(dir, "/R/helpers_data.R"))                                        # find_L, datagen
source(paste0(dir, "/R/helpers_nimble.R"))                                      # nimble_constants, nimble_data_years, nimble_inits
source(paste0(dir, "/R/helpers_sampler.R"))                                     # make_conf, run_sampler
source(paste0(dir, "/R/helpers_summary.R"))                                     # posterior_summary, data_censoring_summary
source(paste0(dir, "/R/model_normal.R"))                                        # model_code
...
for (i in seq_len(nrow(sim_grid_filt))) {
  ...
  ## inner loop over L truncation values with the same dataset
  for (L in config[["L_values"]]) {
    
    ## if any L values have already been fit, load and skip this value
    if (any(
      completed_fits$sim_id == sim_id & completed_fits$L == L)) {
      message("Skipping completed fit: sim_id = ",  sim_id, ", L = ", L)
      next
    }

    ## set seed again
    set.seed(as.numeric(config[["sim_seed"]]) + 1000000 + sim_id*1000 + L)
    ...
    fit <- run_sampler(model,
                       sampler_type = as.character(config[["sampler"]]),
                       pz = pz,
                       L = L,
                       niter = config[["niter"]],
    ...
    if (save_level == 2) {
      ...
      full_file <- file.path(
        sample_dir,
        sprintf("all_variables_array_%03d_L_%03d.csv", idx, L)
      )
    ...
    if (save_level == 3) {
      ...
      trace_file <- file.path(sample_dir, sprintf("trace_samples_sim_%05d_L_%03d.rds", sim_id, L))
      saveRDS(fit$samples, file = trace_file)
    }
    
    ## mark fit completed
    completed_row <- data.frame(sim_id = sim_id, L = L)
    write.table(
    ...
    ## data cleanup to free up memory after each L fit
    rm(fit, model, posterior_summaries, Nconstants, Ndata, Ninits)
  ...
  ## data cleanup to free up memory after each data (over all L values) fit
  rm(data)
