...
dg_glm <- function(
  ...
  censoring_pars <- switch(
    censoring_key,
    "0.2" = list(
      alpha_start = c(1.5, 0.4, 1),
      alpha_width = c(1.1, 2, -0.2),
      eps_obs = 2.1
    ),
    "0.4" = list(
      alpha_start = c(2, 1, 1),
      alpha_width = c(1, 1.5, -0.2),
      eps_obs = 1.5
    ),
    "0.8" = list(
      alpha_start = c(2.7, 1.4, 1.1),
      alpha_width = c(0.9, 0.5, -0.3),
      eps_obs = 1.1
    ),
  )
...
# normal - identity
```{r}
data <- dg_glm(family = Gamma(link = 'log'),
               n = 1000,
               censoring = 0.2)

norm_dat <- data$data

fit <- BCCglm(data = norm_dat,
...
# test with GELc datagen
```{r}
gelc_data <- datagen_gelc_gamma()
list2env(gelc_data, envir = .GlobalEnv)

gamma_dat <- data.frame(Y = Y,
                        X1 = X1,
                        Z = Z,
                        CL = CL,
                        CR = CR)

fit_gamma <- BCCglm(data = gamma_dat,
                    outcome_formula = Y ~ 0 + X1 + Z,
                    family = Gamma(link = 'log'),
                    covariate_formula = Z ~ 0 + X1,
                    censored_covariate = "Z",
                    censoring_bounds = c("CL", "CR"))
# one fit - good posterior mean for betas with GELc gamma DGP
```
